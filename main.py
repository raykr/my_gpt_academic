    ...
    with gr.Blocks(title="GPT 学术优化", theme=set_theme, analytics_enabled=False, css=advanced_css) as app_block:
        ...
        with gr_L1():
            ...
            with gr_L2(scale=1, elem_id="gpt-panel"):
                ...
                with gr.Accordion("prompt区", open=True, elem_id="prompt-panel") as area_prompt:
                    prompt_dropdown = gr.Dropdown(label="选择Prompt", choices=list(load_prompts().keys()),elem_classes='normal_mut_select', interactive=True).style(container=False)
                    prompt_name_textbox = gr.Textbox(label="Prompt名称", placeholder="为这个Prompt命名或选择已存在的Prompt进行修改").style(container=False)
                    prompt_textbox = gr.Textbox(label="当前使用的Prompt", placeholder="在这里输入或修改Prompt", lines=5).style(container=False)
                    save_button = gr.Button("保存Prompt",variant="secondary")
        ...
        def save_prompt(prompt, name):
            prompt_dict = load_prompts()  # 每次都重新读取
            prompt_dict[name] = prompt
            with open(prompt_file,"w") as file:
                for k,v in prompt_dict.items():
                    file.write(f"{k},{v}\n")
            return gr.Dropdown.update(choices=list(prompt_dict.keys()))

        def display_prompt(selected_name):
            prompt_dict = load_prompts()  # 每次都重新读取
            if selected_name and selected_name in prompt_dict:
                return prompt_dict[selected_name],gr.Textbox.update(selected_name)
            return "", gr.Textbox.update(selected_name)
        save_button.click(
            fn=save_prompt,
            inputs=[prompt_textbox, prompt_name_textbox],
            outputs=[prompt_dropdown]
        )
        prompt_dropdown.select(
            fn=display_prompt,
            inputs=[prompt_dropdown],
            outputs=[prompt_textbox, prompt_name_textbox]
        )
        # 终止按钮的回调函数注册
